...
function createIdentity(principal) {
    const {id, account, organization, token, role, userType, name, permissions} = principal;
...
export function login({id, account, organization, token, role, userType, name, permissions}) {
    sessionStorage.setItem(AUTHENTICATED_KEY, JSON.stringify(true));
    sessionStorage.setItem(
        PRINCIPAL_KEY,
        JSON.stringify({id, account, organization, token, role, userType, name, permissions})
    );
